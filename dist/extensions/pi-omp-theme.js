...
function renderBoxedToolCall(theme, toolName, detailLines, options = {}) {
  ...
  return {
    ...
    render(width) {
      ...
      cache2 = { width, lines: dropOmittedLines(lines) };
      return cache2.lines;
    }
  };
}
...
function parseGrepOutput(rawText) {
  ...
  for (const line of stripNotices(rawText)) {
    ...
    if (!file || lineNo === void 0 || content === void 0) continue;
...
function formatMatchRow(theme, match) {
...
function bashTreeHeader(theme, cls, counts) {
  ...
  if (counts) {
    if (cls.kind === "grep") {
      ...
      middle = ` ${theme.fg("accent", `${matches} ${pluralForm("match", matches)}`)}${theme.fg("dim", ` \xB7 ${files} ${pluralForm("file", files)}`)}`;
    } else {
      const files = counts.files ?? 0;
...
function renderBashTreeLines(theme, state, width) {
  const safeWidth = Math.max(1, width);
  const cls = state.cls;
...
function registerGrepCall(toolCallId, pattern, label) {
...
function formatGrepHeader(theme, state) {
  ...
  const label = bold2(theme, "Grep:");
  const patternPart = state.pattern ? ` ${theme.fg("text", state.pattern)}` : "";
  const pathPart = state.pathLabel ? theme.fg("dim", ` \xB7 in ${state.pathLabel}`) : "";
  if (state.isError) {
    return `${icon}${theme.fg("error", bold2(theme, "\u2718 Grep:"))}${state.pattern ? ` ${theme.fg("error", state.pattern)}` : ""}${pathPart}`;
  }
  if (state.matches === void 0) {
  ...
  const filesPart = theme.fg("dim", ` \xB7 ${fileCount} ${pluralForm("file", fileCount)}`);
...
function renderGrepPanelLines(theme, state, width) {
  const safeWidth = Math.max(1, width);
...
function renderGrepPanel(theme, toolCallId) {
  const state = grepPanels.get(toolCallId);
  return {
    invalidate() {
    },
...
var grepTool = {
  call(args, theme, context) {
    noteExecutionStart(context);
    const pattern = String(args?.pattern ?? "");
    registerGrepCall(context.toolCallId, pattern, pathLabel2(String(args?.path ?? ".")));
...
function createDoctor(state) {
  return Object.freeze({
    ...
    ...state.operational?.compatibility && typeof state.operational.compatibility.versionRange === "string" ? { piVersionRange: state.operational.compatibility.versionRange } : {},
...
var StartupComponent = class {
  ...
  overlay;
  requestRender;
  constructor(theme, config, snapshot, overlay, requestRender) {
    this.theme = theme;
    this.config = config;
    this.snapshot = snapshot;
...
function readRecentSessions(sessionFile, limit, nowMs = Date.now()) {
  ...
  try {
    ...
    for (const candidate of candidates) {
      ...
      let head = "";
      try {
        head = readFileSync2(candidate.path, "utf8").slice(0, HEAD_BYTES);
      } catch {
        continue;
      }
...
function detectPiVersion(resolution = {}) {
  ...
  const readFile2 = resolution.readFile ?? ((path) => readFileSync3(path, "utf8"));
  try {
    ...
    recordSnapshots: Object.freeze(
      records.map(
        (record3) => Object.freeze({
          ...
          versionRange: record3.versionRange,
...
function createConfigSourceAdapter(pi, port, paths) {
  ...
  return {
    ...
    async load() {
      ...
      const project = trusted ? await readScopedConfig(port, storage.projectPath) : void 0;
